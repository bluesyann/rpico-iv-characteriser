...
async def serial_write(channels:list):
    ...
    while True:
        ...
        await asyncio.sleep(1/sampling_freq)  # Send message every second
            ...
            if data is not None:
                ...
                while '\n' in serial_buffer:
                    ...
                    if '\n' in serial_buffer:
                        ...
                        break
        await asyncio.sleep(0.001)  # Check for incoming data every 100 ms
...
async def regulator(ch:dict, pwm:PWM):
    ...
    while True:
        ...
        # PWM Regulation Logic (Simple P-Regulator for demonstration)
        m=0
        n=0
        if ch['I_SetPoint'] is None:
            ...
            se= ch['I_SetPoint'] - i #error signal
        ...
        if True:
            ...
            print("---------------------------")
        await asyncio.sleep(REGULATOR_SLEEP_TIME)
...
async def main():
    ...
    Ch1={
        ...
        'nv': 3e-2, # proportional gain for voltage regulation
        'pv': 1e-4, # derivative gain for voltage regulation
        'mi': 1e-4, # integral gain for current regulation
        'ni': 0*1e-2, # proportional gain for current regulation
        'pi': 0*1e-4, # derivative gain for voltage regulation
        'Id': 'Ch1',
